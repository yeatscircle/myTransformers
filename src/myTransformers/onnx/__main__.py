...
def export_with_optimum(args):
    if is_optimum_available():
        ...
        if parsed_optimum_version < version.parse(MIN_OPTIMUM_VERSION):
            raise RuntimeError(
                f"myTransformers.onnx requires optimum >= {MIN_OPTIMUM_VERSION} but {optimum_version} is installed. You "
                "can upgrade optimum by running: pip install -U optimum[exporters]"
            )
    else:
        raise RuntimeError(
            "myTransformers.onnx requires optimum to run, you can install the library by running: pip install "
            "optimum[exporters]"
        )
    cmd_line = [
        sys.executable,
    ...
    logger.info(
        ...
        "myTransformers.onnx is deprecated, and will be removed in v5. You can find more information here: "
...
def export_with_transformers(args):
    ...
    if model_kind in ENCODER_DECODER_MODELS:
        ...
        warnings.warn(
            "The export was done by myTransformers.onnx which is deprecated and will be removed in v5. We recommend"
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use myTransformers.onnx instead of optimum.exporters.onnx to perform the ONNX export. It can be "
            "useful when exporting a model supported in myTransformers but not in optimum, otherwise it is not "
            "recommended."
        ),
    )
...
if __name__ == "__main__":
    logger = logging.get_logger("myTransformers.onnx")  # pylint: disable=invalid-name
