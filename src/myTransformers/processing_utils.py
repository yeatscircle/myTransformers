...
class ProcessorMixin(PushToHubMixin):
    ...
    def _get_arguments_from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        the `myTransformers` root, or they must be custom code that has been registered with the relevant autoclass,
    ...
    def get_possibly_dynamic_module(module_name):
        ...
        for lookup_location in lookup_locations:
            ...
            raise ValueError(
                f"Could not find module {module_name} in `myTransformers`. If this is a custom class, "
...
def _validate_images_text_input_order(images, text):
    ...
    if (images is None and text_is_images) or (text is None and images_is_text) or (images_is_text and text_is_images):
        logger.warning_once(
            ...
            "The `images` and `text` inputs will be swapped. This behavior will be deprecated in myTransformers v4.47."
        )
        return text, images
