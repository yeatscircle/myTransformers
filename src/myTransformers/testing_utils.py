...
from myTransformers import logging as transformers_logging
    ...
    if eetq_available:
        try:
            ...
            if "shard_checkpoint" in str(exc):
                # EETQ 1.0.0 is currently broken with the latest myTransformers because it tries to import the removed
...
def LoggingLevel(level):
    """
    This is a context manager to temporarily change myTransformers modules logging level to the desired value and have it
...
class TestCasePlus(unittest.TestCase):
    ...
    - `pathlib` objects (all fully resolved):
       ...
       - `src_dir` - the directory of `src` (i.e. where the `myTransformers` sub-dir resides)
    ...
    def python_one_liner_max_rss(self, one_liner_str):
        ...
        ```
        one_liner_str = 'from myTransformers import AutoModel; AutoModel.from_pretrained("google-t5/t5-large")'
