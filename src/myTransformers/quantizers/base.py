...
class HfQuantizer(ABC):
    """
    Abstract class of the HuggingFace quantizer. Supports for now quantizing HF myTransformers models for inference and/or quantization.
    This class is used only for myTransformers.PreTrainedModel.from_pretrained and cannot be easily used outside the scope of that method
    yet.

    Attributes
        quantization_config (`myTransformers.utils.quantization_config.QuantizationConfigMixin`):
    ...
    def get_special_dtypes_update(self, model, torch_dtype: "torch.dtype") -> Dict[str, "torch.dtype"]:
        ...
        Args:
            model (`~myTransformers.PreTrainedModel`):
    ...
    def validate_environment(self, *args, **kwargs):
        ...
        passed in `from_pretrained`. You need to define it for all future quantizers that are integrated with myTransformers.
...
class SequentialLlama4TextExperts(ModuleList):
    ...
    def __init__(self, config):
        from myTransformers.models.llama4.modeling_llama4 import Llama4TextMLP
