...
2. Convert to myTransformers
rm -rf olmoe; mkdir olmoe; python /data/niklas/myTransformers/src/myTransformers/models/olmoe/convert_olmoe_weights_to_hf.py --input_dir /data/niklas/llm/checkpoints/olmoe-8x1b-newhp-newds-final-annealFrom1200000_step23842-unsharded --tokenizer_json_path /data/niklas/llm/checkpoints/olmoe-step1200000-unsharded/tokenizer.json --output_dir olmoe
3. Load model via:
```
from myTransformers import OlmoeForCausalLM, AutoTokenizer
import torch
model = OlmoeForCausalLM.from_pretrained("../myTransformers/olmoe", torch_dtype=torch.bfloat16).cuda()
model = OlmoeForCausalLM.from_pretrained("../myTransformers/olmoe").cuda()
tokenizer = AutoTokenizer.from_pretrained("../myTransformers/olmoe")
...
from myTransformers import OlmoeConfig, OlmoeForCausalLM
