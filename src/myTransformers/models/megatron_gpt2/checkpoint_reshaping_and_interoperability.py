...
from myTransformers import AutoTokenizer, GPT2Config
from myTransformers.modeling_utils import WEIGHTS_INDEX_NAME, WEIGHTS_NAME
from myTransformers.utils import check_torch_load_is_safe
...
tensor_parallel_params = [
    ...
    # myTransformers layers to split across tp ranks
    "attn.c_attn.weight",
    "attn.c_attn.bias",
    "attn.c_proj.weight",
    "mlp.c_fc.weight",
...
def merge_transformers_sharded_states(path, num_checkpoints):
    """
    Merge sharded checkpoints from myTransformers into a single checkpoint.
...
def convert_checkpoint_from_megatron_to_transformers(args):
    ...
    for pp_rank in range(pp_size):
        ...
        for key, val in get_element_from_dict_by_path(tp_state_dicts[0], path).items():
            ...
            # Transpose the QKV matrix.
                ...
                # Megatron stores (3*D) x D but myTransformers-GPT2 expects D x 3*D.
    ...
    # For LM head, myTransformers' wants the matrix to weight embeddings.
...
def convert_checkpoint_from_transformers_to_megatron(args):
    ...
    # load the myTransformers model state dict and config
        ...
        for layer in range(num_layers):
            ...
            for layer_name in layers_to_copy:
                ...
                # handle attention K, V, Q weights
                    ...
                    # myTransformers stores D X (3*D) but Megatron-LM expects (3*D) X D.
