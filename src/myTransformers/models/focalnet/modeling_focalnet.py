...
# Copied from myTransformers.models.beit.modeling_beit.BeitDropPath with Beit->FocalNet
...
class FocalNetForMaskedImageModeling(FocalNetPreTrainedModel):
    ...
    def forward(
        ...
        Examples:
        ```python
        >>> from myTransformers import AutoImageProcessor, FocalNetConfig, FocalNetForMaskedImageModeling
        >>> import torch
...
class FocalNetForImageClassification(FocalNetPreTrainedModel):
    # Copied from myTransformers.models.swin.modeling_swin.SwinForImageClassification.__init__ with Swin->FocalNet, swin->focalnet
    def __init__(self, config):
        super().__init__(config)
