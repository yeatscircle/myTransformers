...
from myTransformers.utils import is_torchao_available
...
class ModuleUtilsMixin:
    ...
    def floating_point_ops(
        ...
        paper](https://arxiv.org/pdf/2001.08361.pdf) section 2.1. Should be overridden for myTransformers with parameter
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        ...
        # For old GC format (myTransformers < 4.35.0) for models that live on the Hub
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        if safe_serialization:
            ...
            # We are entering a place where the weights and the myTransformers configuration do NOT match.
            ...
            if len(error_names) > 0:
                raise RuntimeError(
                    f"The weights trying to be saved contained shared tensors {error_names} that are mismatching the myTransformers base configuration. Try saving using `safe_serialization=False` or remove this tensor sharing.",
                )
