...
# `myTransformers` requires `torch>=1.11` but this variable is exposed publicly, and we can't simply remove it.
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name in self._object_missing_backend.keys():
            ...
            if name not in self._class_to_module:
                module_name = f"myTransformers.{name}"
            else:
                module_name = self._class_to_module[name]
                if not module_name.startswith("myTransformers."):
                    module_name = f"myTransformers.{module_name}"
...
def direct_transformers_import(path: str, file="__init__.py") -> ModuleType:
    """Imports myTransformers directly
...
def create_import_structure_from_path(module_path):
    ...
    Here's an example of an input import structure at the src.myTransformers.models level:
...
def spread_import_structure(nested_import_structure):
    ...
    Here's an example of an output import structure at the src.myTransformers.models level:
...
def clear_import_cache():
    ...
    """
    # Get all myTransformers modules
    transformers_modules = [mod_name for mod_name in sys.modules if mod_name.startswith("myTransformers.")]
    ...
    # Force reload main myTransformers module
    if "myTransformers" in sys.modules:
        main_module = sys.modules["myTransformers"]
