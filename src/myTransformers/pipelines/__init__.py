...
def get_task(model: str, token: Optional[str] = None, **deprecated_kwargs) -> str:
    ...
    if getattr(info, "library_name", "myTransformers") not in {"myTransformers", "timm"}:
        raise RuntimeError(f"This model is meant to be used with {info.library_name} not with myTransformers")
...
def clean_custom_task(task_info):
    ...
    task_info["pt"] = tuple(getattr(myTransformers, c) for c in pt_class_names)
    tf_class_names = task_info.get("tf", ())
    if isinstance(tf_class_names, str):
        tf_class_names = [tf_class_names]
    task_info["tf"] = tuple(getattr(myTransformers, c) for c in tf_class_names)
...
def pipeline(
    ...
    **kwargs,
    ...
    ```python
    >>> from myTransformers import pipeline, AutoModelForTokenClassification, AutoTokenizer
