...
7. Check that everything looks correct by uploading the package to the pypi test server:
   ...
   pip install -i https://testpypi.python.org/pypi myTransformers

   Check you can run the following commands:
   python -c "from myTransformers import pipeline; classifier = pipeline('text-classification'); print(classifier('What a nice release'))"
   python -c "from myTransformers import *"
...
# Remove stale myTransformers.egg-info directory to avoid https://github.com/pypa/pip/issues/5466
stale_egg_info = Path(__file__).parent / "myTransformers.egg-info"
if stale_egg_info.exists():
    print(
        (
            "Warning: {} exists.\n\n"
            "If you recently updated myTransformers to 3.0 or later, this is expected,\n"
            "but it may prevent myTransformers from installing in editable mode.\n\n"
...
# 2. once modified, run: `make deps_table_update` to update src/myTransformers/dependency_versions_table.py
_deps = [
    "Pillow>=10.0.1,<=15.0",
    "accelerate>=0.26.0",
    "av",
    ...
    "ruff==0.11.2",
    # `sacrebleu` not used in `myTransformers`. However, it is needed in several tests, when a test calls
...
# since we save this data in src/myTransformers/dependency_versions_table.py it can be easily accessed from
# anywhere. If you need to quickly access the data from this table in a shell, you can do so easily with:
#
# python -c 'import sys; from myTransformers.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets
#
# Just pass the desired package names to that script as it's shown with 2 packages above.
#
# If myTransformers is not yet installed and the work is done from the cloned repo remember to add `PYTHONPATH=src` to the script above
#
# You can then feed this for example to `pip`:
#
# pip install -U $(python -c 'import sys; from myTransformers.dependency_versions_table import deps; \
# print(" ".join([deps[x] for x in sys.argv[1:]]))' tokenizers datasets)
#
...
class DepsTableUpdateCommand(Command):
    ...
    user_options = [
        ...
        ("dep-table-update", None, "updates src/myTransformers/dependency_versions_table.py"),
    ]
    ...
    def run(self):
        ...
        target = "src/myTransformers/dependency_versions_table.py"
...
# when modifying the following list, make sure to update src/myTransformers/dependency_versions_check.py
install_requires = [
...
setup(
    ...
    author_email="myTransformers@huggingface.co",
    ...
    url="https://github.com/yeatscircle/myTransformers",
    package_dir={"": "src"},
    ...
    entry_points={"console_scripts": ["myTransformers-cli=myTransformers.commands.transformers_cli:main"]},
