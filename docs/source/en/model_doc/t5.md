...
```bash
echo -e "translate English to French: The weather is nice today." | myTransformers-cli run --task text2text-generation --model google-t5/t5-base --device 0
```
...
# pip install torchao
import torch
from myTransformers import TorchAoConfig, AutoModelForSeq2SeqLM, AutoTokenizer
