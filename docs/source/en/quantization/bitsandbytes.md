...
```bash
pip install --upgrade myTransformers accelerate bitsandbytes
```
...
model_double_quant = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-13b-chat-hf", torch_dtype="auto",
...
```python
from myTransformers import AutoModelForCausalLM, BitsAndBytesConfig, AutoTokenizer
